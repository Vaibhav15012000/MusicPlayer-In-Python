...
statusbar = ttk.Label(root, text="Welcome to Bakata Music Player", anchor=W, font='Arial 8 bold')
statusbar.pack(side=BOTTOM, fill=X)
statusbar1 = ttk.Label(root, text="Welcome to Bakata Music Player", anchor=W, font='Arial 8 bold')
...
root.title("Bakata Music Player")
ltframe = Frame(root,bg="brown2")
ltframe.pack(side=LEFT, padx=30, pady=30)
songplaylistcontainer = Listbox(ltframe,fg='white')
songplaylistcontainer["bg"]="Black"
...
root.style.configure('TButton', background='lightsteelblue')
rtframe = Frame(root,bg="Black")
rtframe.pack(pady=30,padx=20)
topframe = Frame(rtframe,bg="Silver")
topframe.pack()
root.style = ttk.Style()
#root.style.theme_use("clam")
root.style.configure('TLabel', background='Orange')
...
def play_music():
    ...
    if paused:
        ...
        try:
            ...
            tkinter.messagebox.showerror('File not found', 'Bakata music player could not find the file. Please select again.')
def stop_music():
    mixer.music.stop()
...
def mute_music():
    ...
    if muted:
        ...
        scale.set(0)
        muted = TRUE
middleframe = Frame(rtframe,bg="brown2")
...
bottomframe = Frame(rtframe,bg="brown2")
